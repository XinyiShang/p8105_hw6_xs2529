...
with the majority of data clustered around the mean. It follows the
normal distribution.
...
values (NA) in the `birthweight` dataset. The dataset comprises
`nrow(birthweight)` observations and `ncol(birthweight)` variables,
identified by the names `names(birthweight)`. As part of the cleaning
process, I converted the variables `babysex`, `malform`, `frace`, and
`mrace` into factors.
...
important in determining baby’s weight. So, I considered mom’s height
...
Then, I conducted cross-validation comparing my proposed model to two
alternative models:
...
However, the model I proposed, which includes predictors `mheight`,
`ppwt`, `ppbmi`, `delwt`, and `wtgain`, displayed the poorest
